...
class SettingsScreen extends StatelessWidget {
  ...
  Widget _buildLanguageOption(BuildContext context, {required String language, required String localeCode}) {
    return ListTile(
      ...
      trailing: BlocBuilder<SettingsCubit, Locale>(
        builder: (context, locale) {
          if (locale.languageCode == localeCode) {
            return const Icon(Icons.check_circle, color: AppColors.prussianBlue);
          } else {
            return const Icon(Icons.radio_button_unchecked, color: AppColors.prussianBlue);
          }
        },
      ),
    );
